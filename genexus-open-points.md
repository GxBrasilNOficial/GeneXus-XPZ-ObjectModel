...
Este arquivo registra pontos em aberto que não bloqueiam o trabalho atual, mas podem ser úteis para retomar em futuras sessões de GeneXus dentro de `C:\Dev`.

## Pontos em Aberto

- Alguns tipos internos e estruturas técnicas do `XPZ` já foram identificados por GUID e comportamento, mas nem todos ainda foram mapeados para um nome oficial humano confirmado.
...
- A diretriz compartilhada em `C:\Dev\AGENTS.md` melhora bastante a descoberta de `C:\Dev\Knowledge`, mas não garante que todo ambiente ou runtime de agente de IA sempre carregue e respeite esse arquivo.
